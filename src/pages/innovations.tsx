...
const mapStateToProps = (state: IState): IInnovationsPageStateProps => {
    const available = innovations.filter((innovation) => !state.settlement.innovations.includes(innovation.id))
    ...
    return {
        ...
        not_available: innovations.filter((innovation) => !available.includes(innovation.id))
    ...
    public render() {
        return (
            <StyledText>
                ...
                <h1>Available</h1>
                <ul>
                    {this.props.available.map((innovation, idx) => this.renderInnovation(innovation, idx, InnovationState.Available))}
                </ul>
                <h1>Not available</h1>
                <ul>
    ...
    private renderInnovation(id: Innovations, idx: number, state: InnovationState) {
        ...
        return (
            <ListItem key={idx}>
                ...
                <Content><FancyButton onClick={this.handleDetailClick.bind(this, id)}>🔍</FancyButton></Content>
                {(state === InnovationState.Available || state === InnovationState.Not_Available) && <Content><FancyButton onClick={this.handleAddClick.bind(this, id)}>Take</FancyButton></Content>}
                {state === InnovationState.Taken && <Content><FancyButton onClick={this.handleRemoveClick.bind(this, id)}>Remove</FancyButton></Content>}
            </ListItem>
        );
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(InnovationsPage);
