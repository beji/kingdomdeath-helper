...
const mapStateToProps = (state: IState): IInnovationsPageStateProps => {
    const available = innovations
        ...
        .filter(
            innovation =>
                !innovation.consequence_of || state.settlement.innovations.includes(innovation.consequence_of),
        )
...
const mapDispatchToProps = (
    dispatch: Dispatch<ShowLayerAction | AddInnovationAction | RemoveInnovationAction>,
): IInnovationsPageDispatchProps => ({
    ...
    public render() {
        return (
            <StyledText>
                <h1>Taken</h1>
                <ul>
                    {this.props.innovated.map((innovation, idx) =>
                        this.renderInnovation(innovation, idx, InnovationState.Taken),
                    )}
                </ul>
                <h1>Available</h1>
                <ul>
                    {this.props.available.map((innovation, idx) =>
                        this.renderInnovation(innovation, idx, InnovationState.Available),
                    )}
                </ul>
                <h1>Not available</h1>
                <ul>
                    {this.props.not_available.map((innovation, idx) =>
                        this.renderInnovation(innovation, idx, InnovationState.Not_Available),
                    )}
                </ul>
            </StyledText>
        )
