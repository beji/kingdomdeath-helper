...
import { Affinity, AffinityTypes, GearSet, IGridSlot, IItem, IItemStat, ISettlement, ISurvivor, Item } from '../../interfaces'
...
const reducer: Reducer<ISettlement, Actions> = (state: ISettlement | undefined, action: Actions): ISettlement => {
  ...
  switch (action.type) {
    ...
    case ActionTypes.UPDATE_GEARSLOT_AFFINITY: {
      ...
      if (typeof currentSurvivor === 'undefined') {
        return state
      }
      const updatedSurvivor: ISurvivor = { ...currentSurvivor }
    ...
    case ActionTypes.UPDATE_GEARSLOT_GEARSET: {
      ...
      let updatedSurvivor: ISurvivor = { ...currentSurvivor }
              ...
              slots
                .map(slot => {
                  ...
                  if (item && item.set) {
                    ...
                    setItems[item.set.id] = setItems[item.set.id] ? [...setItems[item.set.id], item.id] : [item.id]
                    return item.set.id
                  }
                })
