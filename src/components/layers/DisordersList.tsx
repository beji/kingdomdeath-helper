...
const mapStateToProps = (state: IState): IDisorderslistStateProps => {
    if (state.interface.layer && state.interface.layer.type === LayerType.disorderlist) {
        if (typeof state.interface.layer.survivor !== 'undefined') {
            ...
            if (survivor) {
                return {
                    currentlySelectedDisorders: survivor.disorders
                        ? survivor.disorders.map(disorder => disorder.id)
                        : [],
                    id: sid,
                }
            }
...
const mapDispatchToProps = (
    dispatch: Dispatch<UpdateSurvivorDisordersAction | HideLayerAction>,
): IDisorderslistDispatchProps => ({
...
class Disorderslist extends React.Component<IDisorderslistProps, IDisorderslistState> {
    ...
    private renderListElement(art: IDisorder) {
        ...
        const isSelected =
            (currentlySelectedDisorders.includes(art.id) && !this.state.disordersToRemove.includes(art.id)) ||
            this.state.disordersToAdd.includes(art.id)
        if (isSelected) {
            return (
                <SelectedListElement onClick={this.deselectDisorder.bind(this, art.id)}>{art.name}</SelectedListElement>
            )
        }
    ...
    private selectDisorder(newArt: Disorders) {
        const currentlySelectedDisorders = this.props.currentlySelectedDisorders || []
        const count =
            currentlySelectedDisorders.length + this.state.disordersToAdd.length - this.state.disordersToRemove.length
        this.setState({
    ...
    private submit(e: SyntheticEvent<HTMLButtonElement>) {
        if (typeof this.props.id !== 'undefined') {
            ...
            const arts = (this.props.currentlySelectedDisorders || [])
                .concat(disordersToAdd)
                .filter(art => !disordersToRemove.includes(art))
