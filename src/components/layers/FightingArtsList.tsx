...
interface IFightingArtslistProps
    extends IFightingArtslistStateProps,
        IFightingArtslistDispatchProps,
        IFightingArtslistOwnProps {}
...
const mapStateToProps = (state: IState): IFightingArtslistStateProps => {
    if (state.interface.layer && state.interface.layer.type === LayerType.fightingartlist) {
        if (typeof state.interface.layer.survivor !== 'undefined') {
            ...
            if (survivor) {
                return {
                    currentlySelectedArts:
                        typeof survivor.fightingArts !== 'undefined' ? survivor.fightingArts.map(art => art.id) : [],
...
const mapDispatchToProps = (
    dispatch: Dispatch<UpdateSurvivorFightingArtsAction | HideLayerAction>,
): IFightingArtslistDispatchProps => ({
...
class FightingArtslist extends React.Component<IFightingArtslistProps, IFightingArtslistState> {
    ...
    private renderListElement(art: IFightingArt) {
        ...
        const isSelected =
            (currentlySelectedArts.includes(art.id) && !this.state.artsToRemove.includes(art.id)) ||
            this.state.artsToAdd.includes(art.id)
        if (isSelected) {
            return (
                <SelectedListElement onClick={this.deselectFightingArt.bind(this, art.id)}>
                    {art.name}
                </SelectedListElement>
            )
        }
    ...
    private submit(e: SyntheticEvent<HTMLButtonElement>) {
        if (typeof this.props.survivor !== 'undefined') {
            ...
            const arts = (this.props.currentlySelectedArts || [])
                .concat(artsToAdd)
                .filter(art => !artsToRemove.includes(art))
