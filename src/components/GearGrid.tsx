            ...
            return (
                <PlayerCard>
                    <PlayerCardHeadline>
                        <div>
                            Player:{' '}
                            <NameEdit name={grid.playername || 'not assigned'} updateFunc={this.handleSetPlayerName} />{' '}
                        </div>
                    ...
                    <GridAffinities>
                        <StyledText>
                            {grid.affinities && grid.affinities.reduce((acc, curr) => (curr === 0 ? acc + 1 : acc), 0)}x{' '}
                            <AffinityIcon affinity={Affinity.red} type={AffinityTypes.grid} />
                        </StyledText>
                        <StyledText>
                            {grid.affinities && grid.affinities.reduce((acc, curr) => (curr === 1 ? acc + 1 : acc), 0)}x{' '}
                            <AffinityIcon affinity={Affinity.green} type={AffinityTypes.grid} />
                        </StyledText>
                        <StyledText>
                            {grid.affinities && grid.affinities.reduce((acc, curr) => (curr === 2 ? acc + 1 : acc), 0)}x{' '}
                            <AffinityIcon affinity={Affinity.blue} type={AffinityTypes.grid} />
                        </StyledText>
                    </GridAffinities>
                    <StyledGrid>
                        {Object.keys(grid.slots).map((v, i, slots) => (
                    ...
                    <GearSets>
                        ...
                        {grid.gearSets && grid.gearSets.length > 0 && (
                            <>
                                Full gearsets:{' '}
                                {grid.gearSets.map((setId: GearSet) => (
                                    <a onClick={() => this.showGearSetDescription(setId)}>{gearSets[setId].name}</a>
                                ))}
                            </>
                        )}
                    </GearSets>
