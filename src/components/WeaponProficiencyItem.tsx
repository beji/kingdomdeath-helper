...
import { ISimpleLayer, IWeaponProficiency, LayerType } from "../interfaces";
...
class WeaponProficiencyItem extends React.Component<IWeaponProficiencyItemProps> {
    ...
    public render() {
        ...
        return (
            <React.Fragment>
                <WeaponProficiencyItemWrapper onClick={this.showDescription}>{weaponproficiency.name}</WeaponProficiencyItemWrapper>
            </React.Fragment>
        );
    }
    private showDescription(e: SyntheticEvent<HTMLElement>) {
        ...
        this.props.showLayer({
            content: weaponproficiency.desc,
            headline: weaponproficiency.name,
            type: LayerType.simple,
        });
    }
}

export default connect(null, mapDispatchToProps)(WeaponProficiencyItem);
