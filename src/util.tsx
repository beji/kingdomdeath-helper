import { ISettlement, SpecialStats } from './interfaces'
...
export function specialStatToString(stat: SpecialStats) {
  switch (stat) {
    ...
    case SpecialStats.bleed_token:
      return 'Bleed Token'
    default:
      return capitalize(SpecialStats[stat])
  }
}
...
export function getURLParam(name: string, urlFragment: string = window.location.href) {
  return decodeURIComponent(urlFragment.replace(new RegExp('^(?:.*[&\\?\\#]' + encodeURI(name).replace(/[\.\+\*]/g, '\\$&') + '(?:\\=([^&]*))?)?.*$', 'i'), '$1'))
}

// T extends {} is needed because of tsx
export const deduplicate = <T extends {}>(arr: ReadonlyArray<T>): ReadonlyArray<T> => arr.filter((item, idx) => idx === arr.indexOf(item))
